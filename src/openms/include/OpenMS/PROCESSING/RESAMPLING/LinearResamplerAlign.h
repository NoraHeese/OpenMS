  ...
  public:
    ...
    */
    ...
    void raster_align(PeakContainerT& container, double start_pos, double end_pos)
    {
      ...
      verifySpacing_(first, last, []<typename PeakTypeIterator>(PeakTypeIterator x){return x->getPos();});
...
protected:
    ...
    void verifySpacing_(PeakTypeIterator it, PeakTypeIterator end, auto access)
    {
      ...
      if (spacing_ < min_dist){
          OPENMS_LOG_WARN << "you have chosen a resampling rate of: " << spacing_ <<\
          "\n this is smaller than the smallest Distance (" << min_dist << ") between Datapoints.\n\
           Since this approximates the dead time of the Detector this can result in fake peaks";
        }
    }
